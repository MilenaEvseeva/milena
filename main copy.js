...
try {
    ...
    var curUserID = 6992666023990938310; // Титлинова
    var curUser = tools.open_doc(curUserID).TopElem;
}
...
function getHistory(user_id, type) {
    ...
    if (type == "total") {
        ...
        for (el in COMPETENCE_APPRAISES) {
            findCompetence = ArrayOptFirstElem(XQuery("for $elem in pas \n\
...
function get_CurUserGoals(queryObjects) {
    ...
    for (el in findTotals) {
        ...
        result.push({
            ...
            access: getAccess(el.person_id, el.boss_id, el.deputy_id, is_close, el.status)
        })
    }
    return tools.object_to_text({
        type: "success",
...
function get_Competence(queryObjects) {
    ...
    var view_type = ArrayOptFirstElem(XQuery("for $elem in assessment_appraises where \n\
        $elem/id = " + appraise_id + " \n\
    return $elem")).status;
    result.is_new_type = view_type == '1' ? false : true;
...
function get_Team(queryObjects) {
        ...
        * \n\
        FROM ( \n\
            SELECT \n\
                ...
                (SELECT COUNT(*) from cc_hc_totals as cc where cc.person_id = col_xml.id and cc.status <> 'Черновик' and cc.is_close <> 1 and cc.global_locked <> 1 and cc.year = @selected_year ) as goals_count, \n\
                ( \n\
                    SELECT \n\
                        case COUNT(pas.id) \n\
                            when 0 then 0 \n\
                    ...
                    FROM \n\
                        pas \n\
                    INNER JOIN assessment_plans as ap on pas.person_id = ap.person_id \n\
                    INNER JOIN assessment_plan as ap_xml on ap.id = ap_xml.id \n\
                    CROSS APPLY \n\
                        ap_xml.data.nodes('assessment_plan') as T(emp)  \n\
                    WHERE \n\
                    pas.person_id = col_xml.id \n\
                    and pas.assessment_appraise_id = @assessment_appraise_id \n\
                    and ( emp.value('workflow_state[1]', 'varchar(max)') = @status_id or @status_id = '') \n\
                    and.pas.assessment_appraise_type = 'competence_appraisal' \n\
                ) as show_competence, \n\
                ( \n\
                    SELECT \n\
            ...
            WHERE \n\
                ...
                and \n\
                ( \n\
                    cmp.value('firstname[1]', 'varchar(max)') like '%'+@search+'%' \n\
                    or \n\
                    cmp.value('lastname[1]', 'varchar(max)') like '%'+@search+'%' \n\
                    or \n\
                    cmp.value('middlename[1]', 'varchar(max)') like '%'+@search+'%' \n\
                ) \n\
            ) as temp \n\
        WHERE \n\
        ...
        * \n\
        FROM ( \n\
            SELECT \n\
                ...
                cmp.value('position_name[1]', 'varchar(max)') as position_name, \n\
                isNULL(cmp.value('pict_url[1]', 'varchar(max)'), @no_avatar) as avatar, \n\
                (SELECT COUNT(*) from cc_hc_totals as cc where cc.person_id = col_xml.id and cc.deputy_id = @boss_id and cc.status <> 'Черновик' ) as goals_count, \n\
                ( \n\
                    SELECT \n\
                        case COUNT(pas.id) \n\
                            when 0 then 0 \n\
    ...
    return tools.object_to_text({
        type: "success",
        message: "",
        data: ArraySort( result, 'This.lastname', '+' )
    }, "json");
}
// Переходы в целях
