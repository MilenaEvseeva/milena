...
function getComments(goal_id) {
    var result = {
        count: 0,
        data: [],
        goal_id: Trim(goal_id)
    };
    var findComment = XQuery("for $elem in blog_entry_comments where $elem/blog_entry_id = "+goal_id+" order by $elem/create_date descending return $elem");
    result.count = ArrayCount(findComment)
    for (el in findComment) {
        result.data.push({
            id: Trim(el.id),
            fullname: Trim(el.person_fullname),
            message: Trim(el.message),
            date: StrLongDate(Date(el.create_date)) + " " + StrTime(el.create_date)
        })
    }
    return result;
}

function get_createComment(queryObjects) {
    var goal_id = queryObjects.HasProperty("id") ? OptInt(queryObjects.id, null) : null;
    var message = queryObjects.HasProperty("message") ? Trim(queryObjects.message) : "";

    if (goal_id != null) {
        newDoc = tools.new_doc_by_name('blog_entry_comment');
        newDoc.TopElem.object_type = "cc_hc_total";
        newDoc.TopElem.message = message;
        newDoc.TopElem.blog_entry_id = goal_id;
        newDoc.TopElem.person_id = curUserID;
        newDoc.TopElem.person_fullname = curUser.fullname;
...
function getGoal(id) {
    ...
    result = {
        ...
        comments: getComments(cardTE.id)
    }
    return result;
}
...
function get_CurUserGoals(queryObjects) {
    ...
    var result = [];
    var is_close_query = year == YEAR ? 12345 : 1
    var findTotals = XQuery("sql: \n\
        ...
        WHERE \n\
            at.person_id = @user_id \n\
            and at.is_close <> "+is_close_query+" \n\
            and at.year = '"+year+"' \n\
    ");
...
function get_Team(queryObjects) {
    ...
    function _getCompetenceStatus(user_id) {
        var findPlan = ArrayOptFirstElem(XQuery("for $elem in assessment_plans where \n\
            $elem/person_id = " + user_id + " \n\
            and $elem/assessment_appraise_id = " + ASSESSMENT_APPRAISE_ID + " \n\
        return $elem"));
        if (findPlan == undefined) {
            ...
            return Trim(findPlan.workflow_state);
        }
    }
    ...
    for (el in findTeams) {
        ...
        result.push({
            ...
            competence_status: _getCompetenceStatus(el.id),
    ...
    for (el in comp_data.leader.items) {
        ...
        if (findComp != undefined) {
            if ( compCard.TopElem.status == 'self' ) {
                ...
                } else if (bossMark == "" || bossMark == null ) {
                    findComp.mark_text = "";
                }
            }
        }
