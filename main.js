...
function get_User(queryObjects) {
    ...
    var category = userCard.custom_elems.ObtainChildByKey('f_ng1k').value;
    var result = {
        id: Trim(user_id),
    ...
    for (el in comp_data.leader.items) {
        ...
        if (findComp != undefined) {
            if ( compCard.TopElem.status == 'self' ) {
                ...
                findComp.mark = el.boss_mark;
                findComp.comment = el.boss_comment;
                finedMarkText = ArrayOptFind(el.scales, 'This.key == ' + XQueryLiteral(el.boss_mark));
